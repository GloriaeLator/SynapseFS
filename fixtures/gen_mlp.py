...
`{prefix}_layers_config.json` is the real `align::topology_parser` "layers"
schema (see layers_config()) -- pass this to `sfs commit --topology`.
`{prefix}_topology.json` is an older, pre-resolved perm_groups/tensors shape
that predates topology_parser.cpp and is kept only because permute.py and
bench/residual_codec.cpp still hand-parse it directly rather than through
the real parser (see topology_sidecar()'s own comment).
